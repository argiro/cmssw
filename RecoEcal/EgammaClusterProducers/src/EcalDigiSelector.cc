...
EcalDigiSelector::EcalDigiSelector(const edm::ParameterSet& ps)
{
  ...
  flagsToSave_ = StringToEnumValue<EcalRecHit::Flags>(ps.getParameter<std::vector<std::string> >("flags_to_save"));
...
void EcalDigiSelector::produce(edm::Event& evt, const edm::EventSetup& es)
{
  ...
  if (pBarrelSuperClusters.isValid() && pEndcapSuperClusters.isValid()){
  ...
  // now, if we want to save digis for rechits with special flags, do it

  if (flagsToSave_.size()){
      edm::Handle<EcalRecHitCollection> prechits;
      const EcalRecHitCollection* rechits=0;
      evt.getByToken(EcalEBRecHitToken_,prechits);
      rechits = prechits.product();
      
      edm::Handle<EBDigiCollection> pdigis;
      const EBDigiCollection* digis=0;
      evt.getByToken(EcalEBDigiToken_,pdigis);
      digis = pdigis.product();

      
      for (auto& rh :  *rechits ){
                  
          if (rh.checkFlags(flagsToSave_)){

              EBDataFrame digi(*digis->find(rh.detid()));
              if (SEEDigiCol->find(rh.detid()) != SEEDigiCol->end()) continue;// we have it
              SEBDigiCol->push_back(rh.detid());
              EBDataFrame df( SEBDigiCol->back());
              for (int iq =0;iq<digi.size();++iq){
                 df.setSample(iq, digi.sample(iq).raw());	      
              }                      
          } // if flag found
      } // for rechits
      ...
      evt.getByToken(EcalEEDigiToken_,peedigis);
      ...
      for (auto& rh :  *rechits ){
          ...
          if (rh.checkFlags(flagsToSave_)){
              EEDataFrame digi(*eedigis->find(rh.detid()));
              if (SEEDigiCol->find(rh.detid()) != SEEDigiCol->end()) continue;// we  have it
              SEEDigiCol->push_back(rh.detid());
              EEDataFrame df( SEEDigiCol->back());
              for (int iq =0;iq<digi.size();++iq){
                 df.setSample(iq, digi.sample(iq).raw());	      
              }                      
